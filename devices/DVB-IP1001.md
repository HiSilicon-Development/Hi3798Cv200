...
本文记录 DVB-IP1001 的器件、板级连接、电气属性以及 Linux 6.12 下的节点对应关系。
...
| DDR | 2 GiB | 板载内存 |
| eMMC | `8GTF4R`，8 GB 标称容量 | 8-bit eMMC 总线，1.8 V I/O，MMC 5.1，DDR52 52 MHz |
...
| USB hub | `1a40:0101` | USB 2.0 高速上行，4 个下游端口 |
...
| 面板指示灯 | `GPIO5_1`、`GPIO5_2` | GPIO41 高有效电源灯、GPIO42 低有效状态灯 |
| 外部接口 | HDMI Type-A、RJ45、USB、CATV 同轴、智能卡、IR、模拟音频 | 各接口连接对应的 SoC 控制器或板载收发器件 |
...
| MAC-PHY 总线 | RGMII；PHY 不启用内部 TX/RX 时钟延迟 |
...
DVB-IP1001 使用一颗 MaxLinear MxL214C 提供四个可独立调谐的 DVB-C 前端。
...
| EHCI | `0xf9890000` | USB 2.0 host；板载 `1a40:0101` hub 接在此路径 |
| xHCI0 | `0xf98a0000` | USB 2.0/USB 3.x root hub |
| xHCI1 | `0xf98b0000` | USB 2.0/USB 3.x root hub；RTL8192EU 使用其 USB 2.0 路径 |

```text
EHCI f9890000 -> USB 2.0 hub 1a40:0101 -> 4 x downstream port
xHCI f98b0000 -> Realtek RTL8192EU 0bda:818b -> Wi-Fi
```

外部接口为 1 x USB 3.x 和 2 x USB 2.0。USB 的 `usbN`、`N-M` 编号由控制器探测顺序生成，不属于固定板级编号。
...
| 电源指示灯 | `GPIO5_1`（legacy GPIO41），高有效 | `/sys/class/leds/:power` |
| 状态指示灯 | `GPIO5_2`（legacy GPIO42），低有效 | `/sys/class/leds/:indicator` |
...
| eMMC | `/soc@f0000000/mmc@9830000` | `f9830000.mmc` | `/dev/mmcblk0`、`mmcblk0p1`..`p14`、`mmcblk0boot0/1`、`mmcblk0rpmb` |
...
| GPIO LED | `/leds/power-indicator`、`/leds/display-indicator` | `gpio-leds` | `/sys/class/leds/:power`、`/sys/class/leds/:indicator` |
...
| USB hub | EHCI 下游设备 | `1a40:0101` | `/sys/bus/usb/devices/N-M` |
